...
export const last_ptr = new ComponentImpl(
  "last_ptr",
  // autoExpandClassSignature currently exposes ref field heaps as List[target].
...
export const findByValueRef = new ComponentImpl(
  "findByValueRef",
  [tyRef(typeVar(0)), tyList(typeVar(0)), tyList(typeVar(0)), tyList(tyInt), tyInt],
  tyRef(typeVar(0)),
  (args) => {
    const start = asRef(args[0]!);
    const nodeHeap = asList(args[1]!);
